...
async function loadLive(){
  ...
  $('real-reason').innerHTML=`当前结论：${esc(dec.real?.reason||'--')}`;$('real-dirbox').innerHTML=`<b>本窗口正在判断是否真实下单</b><br>先看触发方向：<b>${esc(dec.trigger_direction_label||'无')}</b>，再在 UP / DOWN 中选择当前更划算的一边。<br>当前选择：<b>${esc(dec.evaluated_direction_label||'未确定')}</b>（${esc(dec.decision_mode||'未确定')}），使用价格：<b>${num(dec.evaluated_ask,4)}</b>。<br><span class="muted">盘口参考：UP=${num(dec.ask_up,4)}，DOWN=${num(dec.ask_down,4)}。下面任一关键条件未通过，就不会提交真实 FOK。</span>`;conds('real-conds',dec.real?.conditions||[]);
...
setPage(pageFromPath());loadAll();setInterval(loadLive,1000);setInterval(()=>{const p=pageFromPath(); if(p==='real')loadReal(); else if(p==='shadow')loadShadow(); else if(p==='logs')loadLogs();},5000);
