...
def _phase_box_condition_subset(phase_num, all_conditions):
    ...
    return [c for c in (all_conditions or []) if c.get("key") in keys]
...
def current_decision_payload(root: Path) -> dict[str, Any]:
    ...
    if tick:
        ...
        real = [
            ...
            c("ask_rule", "报价上限", "pass" if best_ask is not None and best_ask < 0.7 else "fail" if best_ask is not None else "unknown", f"best_ask={best_ask if best_ask is not None else '--'} < 0.7"),
    ...
    if not decision_pending and intent_allowed is True:
        real.extend([
            c("kelly", "下注金额", "pass" if real_target is not None and real_target >= 2.5 else "fail" if "Kelly<2.5" in real_status else "unknown", f"Kelly建议={cw.get('real_kelly_raw_quote','--')}；目标={real_target if real_target is not None else '--'}；最终最低={max(2.5, float(cw.get('real_platform_min_quote') or 0)) if cw.get('real_platform_min_quote') is not None else 2.5}；窗口boost可用={cw.get('real_min_abs_boost_available','--')}；本次已用={cw.get('real_min_funding_boost_used_this_decision','--')}"),
            c("cap", "资金上限", "pass" if cap is not None and cap >= 2.5 else "fail" if cap is not None else "unknown", f"档位={cw.get('sizing_tier') or 'base'}；fraction={sizing_fraction:.2f}；lifecycle_ratio={max_stake_ratio:.0%}；lifecycle_cap={cw.get('real_window_ratio_cap','--')}；global_cap={cw.get('real_global_window_cap','--')}；实际cap={cw.get('real_hard_window_cap', cap if cap is not None else '--')}；最低单override={cw.get('real_platform_min_global_cap_override_used', False)}；绝对cap={cw.get('real_max_window_risk_usdc','--')}" if cap is not None else "真实余额不可用"),
            c("platform_min", "平台最小单", "pass" if real_target is not None and cw.get('real_platform_min_quote') is not None and real_target >= float(cw.get('real_platform_min_quote') or 0) else "fail" if cw.get('real_platform_min_quote') is not None else "unknown", f"platform_min={cw.get('real_platform_min_quote','--')}；5shares金额={cw.get('real_min_5_shares_quote','--')}；boost可用={cw.get('real_platform_min_boost_available','--')}；本次已用={cw.get('real_platform_min_boost_used','--')}；global_cap={cw.get('real_global_window_cap','--')}；override={cw.get('real_platform_min_global_cap_override_used', False)}；ratio_cap={cw.get('real_ratio_limited_cap','--')}"),
    ...
    shadow = [
        ...
        c("ask_rule", "报价上限", "pass" if ask is not None and ask < 0.7 else "fail" if ask is not None else "unknown", f"ask={ask if ask is not None else '--'} < 0.7"),
